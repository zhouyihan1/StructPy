...
class AdvancedDisplacementCalculationSystem(QMainWindow):
    def __init__(self):
        ...
        # 创建M1转角标签页
        self.tab_m1_corner = QWidget()
        tab_m1_corner_layout = QVBoxLayout(self.tab_m1_corner)
        self.canvas_m1_corner = MplCanvas(self.tab_m1_corner)
        tab_m1_corner_layout.addWidget(self.canvas_m1_corner)
        self.tab_widget.addTab(self.tab_m1_corner, "M1转角")
        ...
        # 创建转角结果图标签页
        self.tab_corner_result = QWidget()
        tab_corner_result_layout = QVBoxLayout(self.tab_corner_result)
        self.canvas_corner_result = MplCanvas(self.tab_corner_result)
        tab_corner_result_layout.addWidget(self.canvas_corner_result)
        self.tab_widget.addTab(self.tab_corner_result, "转角结果图")
        ...
        # 创建A点转角位移计算标签页
        self.tab_a_corner_displacement = QWidget()
        tab_a_corner_displacement_layout = QVBoxLayout(self.tab_a_corner_displacement)
        
        # 转角位移结果显示区域
        self.corner_result_label = QLabel("A点转角位移计算结果将显示在这里")
        self.corner_result_label.setAlignment(QtCore.Qt.AlignCenter)
        self.corner_result_label.setObjectName("cornerResultLabel")
        self.corner_result_label.setWordWrap(True)
        
        tab_a_corner_displacement_layout.addWidget(self.corner_result_label)
        self.tab_widget.addTab(self.tab_a_corner_displacement, "A点转角位移计算")
    ...
    def on_calculate(self):
        try:
            ...
            self.draw_m1_corner_plot(l_val, h_val)
            self.draw_mp_plot(l_val, h_val, q_val, f_val, fb_val)
            self.draw_corner_result_plot(l_val, h_val, q_val, f_val, fb_val ,e_val, i_val    )
            self.calculate_displacement(l_val, h_val, q_val, f_val, fb_val, e_val, i_val)
            self.calculate_corner_displacement(l_val, h_val, q_val, f_val, fb_val, e_val, i_val)
    ...
    def draw_m1_corner_plot(self, l, h):
        """绘制M1转角图"""
        ax = self.canvas_m1_corner.axes
        ax.clear()
        ...
        # 绘制从(0,-h/3)连接B点的红色线
        point_on_ac = (0, -h/3)
        ax.plot([point_on_ac[0], B[0]], [point_on_ac[1], B[1]], 'r-', linewidth=2, label='M1转角线')

        # 在红色线上标记点
       
        ax.text(point_on_ac[0]-0.8, point_on_ac[1], f'{1/l:.2f}', fontsize=12, color='red', fontweight='bold')

        # 添加点的标签
        ax.text(A[0]-0.4, A[1]+0.3, 'A', fontsize=14, color='#1e40af', fontweight='bold')
        ax.text(B[0]+0.2, B[1]+0.2, 'B', fontsize=14, color='#1e40af', fontweight='bold')
        ax.text(C[0]-0.4, C[1], 'C', fontsize=14, color='#1e40af', fontweight='bold')

        # 在A点添加顺时针箭头
        # 圆弧圆心是A点，从60度到130度（1/4周长）
        arc_radius_a = 0.4  # 圆弧半径
        arc_a = Arc(A, 2*arc_radius_a, 2*arc_radius_a, angle=0, theta1=60, theta2=130, color='blue', linewidth=1.2)
        ax.add_patch(arc_a)

        # 在60度端添加箭头（顺时针方向）
        arrow_angle = 60  # 箭头位置角度
        arrow_start_a = np.array(A) + np.array([arc_radius_a * np.cos(np.radians(arrow_angle + 5)),
                                               arc_radius_a * np.sin(np.radians(arrow_angle + 5))])
        arrow_end_a = np.array(A) + np.array([arc_radius_a * np.cos(np.radians(arrow_angle)),
                                             arc_radius_a * np.sin(np.radians(arrow_angle))])
        arrow_head_a = FancyArrowPatch(arrow_start_a, arrow_end_a,
                                      arrowstyle='->', mutation_scale=15, color='blue', linewidth=1.2)
        ax.add_patch(arrow_head_a)

        # 设置图形属性
        ax.set_xlabel('X 坐标 (m)', fontsize=12)
        ax.set_ylabel('Y 坐标 (m)', fontsize=12)
        ax.set_title('M1转角图', fontsize=16, fontweight='bold')
    ...
    def draw_corner_result_plot(self, l, h, q, F, FB, e, i):
        """绘制转角结果图（照搬位移图结构，加上转角箭头和标注）"""
        ax = self.canvas_corner_result.axes
        ax.clear()
        ...
        # 在A点上方添加转角箭头
        arc_radius_a = 0.4  # 圆弧半径
        arc_a = Arc(A_deformed, 2*arc_radius_a, 2*arc_radius_a, angle=0, theta1=60, theta2=130, color='green', linewidth=2)
        ax.add_patch(arc_a)

        # 在60度端添加箭头（顺时针方向）
        arrow_angle = 60  # 箭头位置角度
        arrow_start_a = np.array(A_deformed) + np.array([arc_radius_a * np.cos(np.radians(arrow_angle + 5)),
                                                        arc_radius_a * np.sin(np.radians(arrow_angle + 5))])
        arrow_end_a = np.array(A_deformed) + np.array([arc_radius_a * np.cos(np.radians(arrow_angle)),
                                                      arc_radius_a * np.sin(np.radians(arrow_angle))])
        arrow_head_a = FancyArrowPatch(arrow_start_a, arrow_end_a,
                                      arrowstyle='->', mutation_scale=12, color='green', linewidth=2)
        ax.add_patch(arrow_head_a)

        # 计算转角值
        corner_displacement = (0.5*l*1*2/3*FB*l + 2/3*l*q*l**2/8*0.5) / ei

        # 标注转角大小
        ax.text(A_deformed[0]+0.2, A_deformed[1]+0.2, f'θ= {corner_displacement:.6f} rad',
        ...
        ax.set_xlabel('X 坐标 (m)', fontsize=10)
        ax.set_ylabel('Y 坐标 (m)', fontsize=10)
        ax.set_title('转角结果图', fontsize=14, fontweight='bold')
        ax.legend(loc='best')
        ...
        y_margin = h * 0.2
        ax.set_xlim(-x_margin, l + x_margin)
        ax.set_ylim(-h - y_margin, 1.2)  # 调整上界限，为转角标注留空间

        self.canvas_corner_result.draw()
    ...
    def calculate_corner_displacement(self, l, h, q, F, FB, e, i):
        """计算A点转角位移"""
        # 将单位转换: E(MPa) * I(mm^4) -> N·mm^2，然后转为kN·m^2
        ei = e * i * 1e-6
        ...
        # 显示结果
        self.corner_result_label.setText(f"A点转角位移计算结果:\n\n"
                                        f"FB = 0.5*q*l + 0.5*F*h*l = {FB:.2f} kN\n\n"
                                        f"转角计算公式:\n"
                                        f"θ = (0.5×l×1×2/3×FB×l + 2/3×l×q×l²/8×0.5) / EI\n\n"
                                        f"其中:\n"
                                        f"- 第一项: 0.5×l×1×2/3×FB×l = {0.5*l*1*2/3*FB*l:.6f}\n"
                                        f"- 第二项: 2/3×l×q×l²/8×0.5 = {2/3*l*q*l**2/8*0.5:.6f}\n"
                                        f"- EI = {ei:.2e} kN·m²\n\n"
                                        f"A点转角: {corner_displacement:.6f} rad")
